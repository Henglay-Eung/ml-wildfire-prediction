...
const precipitationScale = d3.scaleThreshold()
    .domain([25.00, 50.00, 75.00, 100.00])
...
function requestDataBody(date) {
    // Ensure we're using noon of the selected date to avoid timezone issues
...
function startVisualization([us, fips]) {
    try {
        ...
        countyPaths.on("mouseover", function(event, d) {
            ...
            const countyName = fips2County[event.id] ? fips2County[event.id] : 'Unknown County';

            
            // Get weather data
            if (!d.weatherData) {
                d.weatherData = weatherData.find(w => w.fips === d.id);
            }

            
            // Get wildfires in this county
            const countyWildfires = wildfireData.filter(w => w.FIPS === d.id);

            
            const wildfireInfo = countyWildfires.length > 0 
                ? countyWildfires.map(w => `<br>Wildfire: ${w.FIRE_SIZE} acres`).join('')
                : '<br>No wildfires';
            
            // Get the selected data type value
            let dataValue = '';
            if (d.weatherData && dataTypeLabel) {
                switch(selectedMapType) {
                    case 'temperature-checkbox':
                        dataValue = formatValue(d.weatherData.tmax, 'temperature');
                        break;
                    case 'rain-checkbox':
                        dataValue = formatValue(d.weatherData.prcp, 'precipitation');
                        break;
                    case 'wind-checkbox':
                        dataValue = formatValue(d.weatherData.wind_speed, 'wind');
                        break;
                    case 'fuel-checkbox':
                        dataValue = formatValue(d.weatherData.fuel, 'fuel');
                        break;
                }
            }


            const tooltipContent = `<strong>${countyName}</strong>` +
                wildfireInfo +
                (dataTypeLabel ? `<br>${dataTypeLabel}: ${dataValue}` : '' );
...
function getCountyColor(d, selectedMapType) {
    ...
    // Cache the weather data for this county
    if (!d.weatherData) {
        d.weatherData = weatherData.find(w => w.fips === fips);
    }
    ...
    switch(selectedMapType) {
        ...
        case 'fuel-checkbox':
            return d.weatherData?.fuel ? fuelColorScale(d.weatherData.fuel) : 'rgb(194,194,194)';
...
function drawWildfireMarkers() {
    ...
    markers.on("mouseover", function(event, d) {
        ...
        const tooltipContent = `<strong>Details</strong><br>` +
            `Wildfire Size: ${event.FIRE_SIZE || 'Unknown'} acres<br>` +
            `Location: ${fips2County[event.FIPS]}`;
...
function createDataLegend(selectedMapType) {
    ...
    if (selectedMapType === 'rain-checkbox') {
        ...
        const precipRanges = [
            { range: "0.00 to 25.00", color: "#deebf7" },
            { range: "25.00 to 50.00", color: "#9ecae1" },
            { range: "50.00 to 75.00", color: "#4292c6" },
            { range: "75.00 to 100.00", color: "#08519c" }
        ];
        ...
        const tempRanges = [
            { range: "32.00 to 47.75", color: "#f2f0f7" },
            { range: "47.75 to 63.50", color: "#cbc9e2" },
            { range: "63.50 to 79.25", color: "#9e9ac8" },
            { range: "79.25 to 95.00", color: "#756bb1" }
        ];
        ...
        if (scale) {
            ...
            items.append("span")
                .text(d => d.value ? `${d.value.toFixed(1)}+` : "0");
        }
    }
}
...
socket.on('data_broadcast', data => {
    ...
    wildfireData = (data.wildfire || []).filter(d => {
        if (!d.LATITUDE || !d.LONGITUDE || !d.FIRE_SIZE) {
            console.warn('Invalid wildfire data record:', d);
            return false;
        }
        return true;
    });
    
    // Process weather data - ensure it has the data we need
    weatherData = (data.weather || []).filter(d => {
        // Basic validation
        if (!d.lat || !d.lon || !d.fips) {
            console.warn('Invalid weather data record (missing coordinates or FIPS):', d);
            return false;
        }
        
        // Ensure precipitation values are numbers
        if (d.prcp !== undefined) {
            ...
            // Log precipitation values to debug
            if (d.prcp > 0) {
                console.log(`County ${d.fips} has precipitation: ${d.prcp}`);
            }
        }
    ...
    // Count counties with precipitation data
    const precipCount = weatherData.filter(d => d.prcp > 0).length;
    console.log(`Counties with precipitation > 0: ${precipCount}`);
    
    // Show a sample weather record
    if (weatherData.length > 0) {
        console.log('Sample weather record:', weatherData[0]);
    }
